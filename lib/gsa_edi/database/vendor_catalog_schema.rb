...
def delete_sip_tables
	...
	if gets.to_s.upcase.include? 'Y'
		ImportTables.each { |i_table| DP.drop_table?(i_table) }
	end
end
...
# manufacture name
	def deduplicate_table(db, table, *columns)
		...
		if !args.empty?
			db.create_table!(:t1, as: db[table].distinct(*columns))
		else
			db.create_table!(:t1, as: db[table].distinct)
		end
		db.create_table!(table, as: db[:t1])
		puts "Duplicates Removed from #{table}"
	end

	def deduplicate_iprod
		deduplicate_table(DP, :adv_prod, :CONTNUM, :MFGPART, :MFGNAME)
	end

	def deduplicate_schedule_tables(db)
	  ImportTables.each { |table| deduplicate_table(db, table) }
	end

	def timestamp_schedule_tables(db)
		ImportTables.each { |table| time_column(db, table) }
	end

def create_vendor_catalog_tables
DP.create_table?(:icolors) do
	column :CONTNUM      ,String,           size: 30               ,null: false      #   Contract number. Format 'GS-99F-9999A' or GS-'GS-99F-999AA' ('V999P-99999 ' or 'V999D-99999 ' for VA contract) in CONTR.TXT.
...
DP.create_table?(:icontr) do
	column :CONTNUM      ,String,           size: 30               ,null: false      #   Contract number. Format 'GS-99F-9999A' or GS-'GS-99F-999AA' ('V999P-99999 ' or 'V999D-99999 'for VA contract), unique.
